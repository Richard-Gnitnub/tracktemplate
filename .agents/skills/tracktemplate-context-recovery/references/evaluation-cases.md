...
| CE-10 merged worktree retirement | Can a merged worktree with tracked cleanliness and ignored files be removed? | [`RECOVERY_AND_BACKUP.md`](../../../../reference/RECOVERY_AND_BACKUP.md#deliberate-worktree-retirement), accepted remote-main identity, the complete local-state inventory, canonical preservation owners, and IDE/workspace activity evidence | Show accepted-history containment, tracked cleanliness, and inactivity. Record ignored and non-ignored local state. Record each classification, proof, necessary preservation, authority, and post-removal diff. | Merge, tracked cleanliness, and Git ignore state give no disposal authority. If ownership or uniqueness is ambiguous, do not remove the worktree. Do not use force, stash, or ad-hoc relocation. |
