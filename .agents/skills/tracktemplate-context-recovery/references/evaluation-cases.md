...
| CE-10 merged worktree retirement | Can a merged, tracked-clean worktree with ignored files be removed? | [`RECOVERY_AND_BACKUP.md`](../../../../reference/RECOVERY_AND_BACKUP.md#deliberate-worktree-retirement), accepted remote-main identity, the complete local-state inventory, canonical preservation owners, and IDE/workspace activity evidence | Prove accepted-history containment, tracked cleanliness, and inactivity. Record ignored and non-ignored local state. Record each classification, proof, required preservation, authority, and post-removal diff. | Do not infer disposal from merge, tracked cleanliness, or Git ignore state. Do not remove the worktree when ownership or uniqueness is ambiguous. Do not use force, stash, or ad-hoc relocation. |
