...
For analytical geometry, use `$tracktemplate-railway-mathematics`. If the document keeps a result
as a FreeCAD object, use
...
FreeCAD `Part` Python wrapper from the OCCT C++ API. Examine each call against
the supported host.
...
FreeCAD/OCCT edges, faces, solids, tessellations, and generated topology stay
...
2. **Make geometry in small steps.** Examine each intermediate result. Preserve
   placements and transformations explicitly. Do not convert coordinates repeatedly. Prevent uncontrolled tolerance accumulation.
3. **Validate risky operations.** After each operation listed below, examine
   the result against the geometry contract. Examine null or empty results,
   kernel validity, type, count, closure, orientation, connectivity,
   degeneracy, dimensions, and railway invariants.
4. **Examine Boolean and feature results.** Find unchanged, fragmented,
   missing, or dimensionally incorrect results. These failures can occur
   without an exception.
5. **Control tolerances.** Record input tolerances and final tolerances. Limit
...
7. **Preserve semantic identity.** Do not persist generated topology names or
   ...
   Use construction records and stable IDs for semantic components. After an
   operation changes topology, its previous mapping becomes invalid.
8. **Validate output.** Use the intended FreeCAD object, STEP/STL/DXF,
...
- Round-trip checks, if necessary
- Scale and placement
...
- Input, final, and permitted tolerances
