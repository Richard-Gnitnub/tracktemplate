...
def validate():
    ...
    assert len(record["function_names"]) == (11 if baseline_only else 14)
    assert record["schema_version"] == (8 if baseline_only else 9)
    ...
    if not baseline_only:
        ...
        for caller, targets in (
            (module.prepare_track_alignment, (proof.PUBLIC[2],)),
            (module.build_platform_core, proof.PUBLIC[:2]),
        ):
