...
| PR-17 | **P:** versioned schemas, bounded ingress and copied-target-only migration. **D:** runtime, detector, corrupt/unsupported, save/reopen and rollback tests. **R:** preserve the source byte-for-byte and fall back to inspection-only/recoverable abort. | Partial | Transition persistence and outer B14/B15 detection pass in [PHASE4_CANONICAL_STATE.md](PHASE4_CANONICAL_STATE.md); no entity-family migrator is qualified. **Next proof:** Phase 4 policy acceptance, family fixtures in Phases 7–9, then Phase 10/11 clean upgrades. |
