...
| The supported schema/version window is agreed and tested | Active — bounded read window accepted; migration support open | On 2026-07-22 the owner accepted exact transition-state v1, chair-package v1 and read-only B14/B15/expected-mixed ingress with all fail-closed limits. The first spacing-matched plain-line transition assessment proves deterministic canonical-input sufficiency and semantic identity without mutation. No entity family is write-qualified; separate owner authority and copied-target migration evidence remain due. |
...
| First extraction slice | Resolved and accepted at Phase 3 closeout on 2026-07-22 — domain/API, all-three-callers, rollback, routed GUI, failure-recovery and contracted performance checks pass; temporary route removal remains a separate Phase 4 entry change | Candidate contracts, bounded closure cuts, workflow/performance profiles, scorecard, [PHASE3_TRANSITION_SLICE.md](PHASE3_TRANSITION_SLICE.md) and owner acceptance |
...
| Supported FreeCAD/Python and legacy document window | Bounded read window accepted on 2026-07-22 for transition-state v1, chair-package v1 and B14/B15/expected-mixed ingress; no family migration authorised | Exact runtime probe, official Addon metadata fields, B14/B15 source/schema anchors, fail-closed detector, first read-only family assessment and project-owner acceptance; copied-target family fixtures remain future authority gates |
...
| PR-17 | **P:** versioned schemas, bounded ingress and copied-target-only migration. **D:** runtime, detector, corrupt/unsupported, save/reopen and rollback tests. **R:** preserve the source byte-for-byte and fall back to inspection-only/recoverable abort. | Partial | The bounded read window is owner-accepted; transition persistence, outer B14/B15 detection and the first read-only canonical-input assessment pass in [PHASE4_CANONICAL_STATE.md](PHASE4_CANONICAL_STATE.md). No entity-family migrator is qualified. **Next proof:** separate owner authority plus an atomic copied-target fixture before advertising the first family, later family fixtures at Phases 7–9, then Phase 10/11 clean upgrades. |
| PR-18 | **P:** named owner and retirement gate for every dual path. **D:** inventory/structural checks and phase-close review. **R:** explicit legacy rollback only until retirement evidence passes. | Partial | [PHASE3_TRANSITION_SLICE.md](PHASE3_TRANSITION_SLICE.md) records the bounded retained comparison route. **Next proof:** its Phase 4 retirement evidence, later per-slice removals, and Phase 10 absence of unowned production duplication. |
