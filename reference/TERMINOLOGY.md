...
| --- | --- |
| Product and tools | **TrackTemplate**, **TrackTemplate Core**, and **Layout Editor** are **product names**. **FreeCAD**, **Coin**, **Python**, **Git**, **GitHub**, **Ruff**, **Templot**, and **PyCharm** are **tool names**. **IDE**, **Addon**, **Workbench**, and **FCMacro** are technical terms for software. A **product** is TrackTemplate or a TrackTemplate component that a person operates. A **tool** is software that does one specified operation. **Software** is a TrackTemplate product or tool. |
| Product operation | **Product behaviour** is a product operation or result that a person can examine. **Host integration** is the product connection to FreeCAD. |
| Bounded scope | The documents `reference/PRODUCT_VISION.md` and `reference/ENGINEERING_POLICY.md` define **railway behaviour**, **export behaviour**, **product performance**, **production**, and **physical output**. |
| Project | The **project** is TrackTemplate product work, repository work, and governance. |
| Authority | **Governance** is the project control of authority, status, evidence, and decisions. The file `reference/ENGINEERING_POLICY.md` defines **Owner view**, **canonical information**, **proof/provenance**, **governance control**, **project authority**, **project owner**, and **competing owner**. |
| Owner-view fields | `TT-DOC-001` defines these six **owner-view field** terms: Current state, **What changed**, **What now works**, **Limitations/findings**, **Owner decision**, or **Next action**. |
| Status | The current records define **Status**, **phase**, **phase exit**, **risk**, **decision**, **decision register**, and **phase evidence**. The current records also define **Finding**, **limitation**, **disposition**, **controlled meaning**, and **acceptance**. A **safety/risk panel** is the Level 3 review in `reference/ENGINEERING_POLICY.md`. A **sequence nonconformance** records an operation that had project authority but did not have the necessary governance sequence first. **Retrospective authority** is a claim that missing authority was available before an operation. A record after the operation cannot give that authority. |
| Risk data | A **risk register** is the canonical record for risks. **Severity**, **treatment**, **deadline**, and **control effectiveness** are risk terms. **Critical**, **High**, **Medium**, and **Low** are **severity values**. **Remove**, **Mitigate**, **Tolerate**, and **Accept** are **treatment values**. A **risk owner** is the person that the risk record names in its `owner` value. A **panel recommendation** is the result that a safety/risk panel gives. |
...
| Identity | An **identity** identifies one item without ambiguity. An **exact state** identifies one complete exact candidate. An **exact Git identity** identifies a Git object without ambiguity. An **exact candidate** is one commit and tree that the author freezes. A **stable identity** does not change when a tool operates again. |
| Document structure | A **document** contains project information. **Technical Documentation Profile** is the exact title for `TT-DOC-001`. **Documentation profile**, **document class**, **canonical document**, **canonical prose**, and **live document** are TrackTemplate document terms. **Frozen history**, **logical unit**, **complete logical unit**, **writing checklist**, **project plan**, and **technical provenance** are also document terms. A document class identifies the applicable content category in the documentation profile. A complete logical unit has the complete information for that unit. |
...
| STE source tools | A **PDF extractor** reads **verified source bytes**. **Source mode** gives one **bounded source excerpt**. The tool binds a **derived cache** to the source identity and can rebuild it. The **source and retrieval procedure** owns local source and STE lookup operation. |
...
| Approved word | A **word** is one unit in the word count. An **approved word** has an **approved meaning** and **part of speech** in the STE dictionary. **Ordinary vocabulary** contains approved words that are not technical terms. A **technical term** is a technical noun or technical verb in the technical-term register. A **registered form** is the full technical term in the technical-term register. A **shorter form** is the approved short name for one technical noun. |
| Part of speech | **Noun**, **verb**, **adjective**, and **adverb** identify the part of speech for an approved word. |
| Sentence grammar | A **sentence** is one unit in the `descriptive` or `procedural` content category. A **grammatical subject** identifies the logical agent or the item that has a condition or result. A **logical agent** is the person, tool, or system that does the operation. **Sentence grammar** is the structure of one sentence. |
| Pronoun antecedent | A **pronoun antecedent** is the word to which a **pronoun** refers. A **multi-word noun** is one noun with one or more **modifier** words. A modifier adds information to a noun. |
| Procedure sentence | A **work step** is one part of a procedure. A **procedure sentence** is one sentence in the `procedural` content category. A **vertical list** is a list with one item on each line. An **instruction** tells the user to do one operation. |
| Condition order | A **condition** is information that is necessary before an instruction. **Condition order** puts the condition before its instruction. **Simultaneous actions** occur at the same time. An **immediate result** occurs directly after an operation. |
...
| Evidence information | **Detailed technical provenance**, **detailed evidence**, **detailed proof**, and **detailed validation** identify information below the canonical information. **Implementation evidence** is source, test, commit, or diff information. It gives no authority for an applicable requirement or decision. |
| Reviewer | **Evidence**, **validation**, **validator**, **recovery validator**, **reviewer**, **author**, and **implementing agent** are TrackTemplate review terms. **Agent**, **independent reviewer**, **independent quality reviewer**, **independent security/recovery reviewer**, and **independent documentation reviewer** are also review terms. **Independent review**, **quality review**, **security/recovery review**, **documentation review**, and **documentation alignment** are review terms. An **assurance** connects evidence and review to one specified claim. A recovery validator is a validator for recovery controls. **Agent context** is the information that an agent uses for one task. |
| Unverified conformance | **Unverified conformance** has no accepted conformance review evidence. |
| Assurance workflow | **Workflow**, **Git workflow**, **context recovery**, **routing**, **handoff**, **skill**, **skill catalog**, and **documentation skill routing** are TrackTemplate workflow terms. **Workflow responsibility**, **competing responsibility**, **authority boundary**, and **non-ownership boundary** are also workflow terms. **Author-side assurance** is a conformance review with full applicability. The author completes it before an independent review. |
| Assurance result | **Conformance review**, **conformance scope**, **semantic control**, **preservation audit**, **complete result**, **review result**, **assessment**, and **machine-verifiable assurance** are TrackTemplate terms for assurance. A **semantic mutation** is a controlled change that a semantic control must reject. A **regression test** contains a check for a problem that a previous exact candidate had. A **substantial cycle** changes Status, a decision, or detailed validation. A **snapshot** is a copy for recovery. The recovery policy defines its controls. |
...
| Recovery checkpoint | A **checkpoint** is named Git state that is necessary before a repository operation with recovery risk. A **clean checkpoint** has tracked cleanliness. A **pushed checkpoint** has a branch on GitHub that contains its commit. |
...
| Workspace recovery | A **workspace** is one worktree that workspace alignment examines. **Workspace alignment** compares a workspace with named Git state. A **worktree map** identifies each worktree in `git worktree list`. **Accepted product state** has acceptance from the project owner. The [recovery policy](RECOVERY_AND_BACKUP.md#visible-recovery-state) is the canonical owner. |
| Worktree retirement | **Worktree retirement** is the complete procedure that an implementing agent uses for removal of one worktree. Git gives **accepted-history containment** when the accepted commit contains the specified commit. **Removal authority** is project authority for one specified worktree or branch operation. |
| Worktree retirement evidence | **Tracked cleanliness** is a Git state with no tracked change in the Git index or worktree. In tracked cleanliness, the Git index has no `assume-unchanged` or `skip-worktree` value. A **local-state inventory** identifies each local file that is not in the Git index. A **retirement plan** classifies the local-state inventory with its recorded SHA-256. A **retirement audit** examines the retirement plan without removal. |
| Local-state types | A **local-state type** is one **classification** for worktree retirement. The 5 local-state types are **authoritative local source**, **retained evidence**, **rebuildable cache/generated state**, **temporary disposable state**, and **ambiguous or uniquely owned state**. **Historical losslessness** is the result of a completed operation that kept all tracked files, authoritative local source, retained evidence, and files with no other copy. |
| Git state | **Git state** is repository information from Git. **Repository**, **worktree**, **branch**, **branch tip**, **branch on GitHub**, **commit**, **accepted commit**, **Git object**, **blob**, and **tree** are software terms. **Git index**, **HEAD**, **parent commit**, **pull request**, **pull-request state**, **draft pull request**, **merge commit**, **tracked file**, **tracked change**, and **protected main** are also software terms. **SHA**, **hash**, **path**, **diff**, and **continuous integration (CI)** are also software terms. |
| Branch tip | A branch tip is the commit at HEAD of the specified branch. An accepted commit is the commit that the project uses for accepted-history containment. A draft pull request has no authority for a merge into protected main. |
| Software data | **File**, **filename**, **directory**, **symbolic link**, **symbolic link loop**, **byte**, **byte size**, **bytecode**, **metadata**, **prefix**, **UTF-8**, **JSON key**, **duplicate key**, **input**, and **test** are software terms. A symbolic link loop is one or more symbolic links in a cycle. An input is data that a tool examines. A **PDF** is an external source file. A **source archive** is a local file from Templot. A **source path** is the local path for the authorised source. |
| Software operation | An **operation** is work that a person, tool, or system does. A user uses a **command** to start one local tool operation. A **version** identifies one tool, schema, or source. A **development tool** is not a **product runtime dependency**. A **diagnostic** is information about a tool result. |
| Syntax check | **Syntax** is the structure of Python or FCMacro source. A **syntax check** is a check for syntax. |
| Software check | A tool does a **check** to examine one **criterion**. |
| Run configuration | A **run configuration** contains data for one PyCharm tool operation. A **validation profile** contains a specified set of checks. The **standalone Python** validation profile does not use FreeCAD. An **environment variable** supplies one specified value to a tool process. A **VCS root** connects a PyCharm project to a repository. |
| Software result | A **command output** contains information that a command returns. A **Git error** is a Git result for an operation that did not complete. A **file-system error** is a local tool result for a file or directory operation that did not complete. A **validation tool** is a tool that does a specified validation check. |
...
| File owner | A **file owner** and **file mode** control local file access. A **user** identifies who starts a local tool. The **current user** starts the STE lookup. **Authentication data** gives access to a system. **root** is the user with all local access. |
...
| **Preserve** | Keep specified repository data, local source, evidence, or authority available without a change. |
...
| **Return** | Give one result from a local tool operation. |
...
| **Report** | Give a result or limitation without authority. |
...
| **Classify** | Put a validation result with `FAIL` in one failure class that the testing policy defines. Put one local-state inventory item in one local-state type that the recovery policy defines. |
