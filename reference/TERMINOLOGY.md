...
| Owner-view fields | `TT-DOC-001` defines the 6 **owner-view field** terms: Current state, **What changed**, **What now works**, **Limitations/findings**, **Owner decision**, and **Next action**. |
...
| STE review record | A **review receipt** records review evidence without certification or endorsement. An **exact-content exclusion** identifies data or an **exact title** that the conformance review does not include. An **author-review worklist** is temporary input for a review receipt. A **review category** identifies part of the conformance review. The `path`, `start_line`, `end_line`, and `sha256` values identify a logical unit in the review. |
...
| Assurance workflow | **Workflow**, **Git workflow**, **context recovery**, **routing**, **handoff**, **skill**, **skill catalog**, and **documentation skill routing** are workflow terms. **Workflow responsibility**, **competing responsibility**, **authority boundary**, and **non-ownership boundary** are also workflow terms. **Author-side assurance** is a conformance review with full applicability. Before the author freezes the exact candidate, a different documentation reviewer uses a **read-only challenge** to examine the author-review worklist and changed prose. The read-only challenge has no acceptance. |
| Assurance result | **Conformance review**, **conformance scope**, **semantic control**, **preservation audit**, **complete result**, **review result**, **assessment**, and **machine-verifiable assurance** are assurance terms. A **semantic mutation** is a change that a semantic control must reject. A **regression test** examines a previous problem. An **unresolved disposition** has no accepted result. A **substantial cycle** changes project state or adds a decision. It can also add detailed validation. |
...
| Recovery checkpoint | A **checkpoint** is named Git state that is necessary before a repository operation with recovery risk. A **clean checkpoint** has tracked cleanliness. A **pushed checkpoint** has a branch on GitHub that contains the checkpoint commit. A **snapshot** is a copy of repository and evidence state under the recovery policy. |
...
| Software operation | A **command invocation** contains an **executable**, **arguments**, and a **working directory**. A tool process uses a **command** to start a local operation. A **version** identifies a tool, schema, or source state. A **development tool** is not a **product runtime dependency**. A **diagnostic** contains information about a tool result. |
