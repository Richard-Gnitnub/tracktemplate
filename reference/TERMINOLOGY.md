...
| Worktree retirement | **Worktree retirement** removes one registered worktree after the recovery policy gives a complete procedure. **Accepted-history containment** shows that the accepted commit contains the target commit. **Tracked cleanliness** means that the target has no tracked index or worktree change and no `assume-unchanged` or `skip-worktree` index flag. A **local-state inventory** contains each ignored and non-ignored local-only file and its identity, and a **retirement plan** classifies that exact inventory. A **retirement audit** examines the plan without removal. An **authoritative local source** and **retained evidence** need preservation. **Ambiguous or uniquely owned state** stops retirement, while **rebuildable cache/generated state** and **temporary disposable state** need a validated disposal basis. |
