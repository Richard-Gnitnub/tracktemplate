...
| Governance state | The current records define **Status**, **phase**, **phase exit**, **risk**, **decision**, **decision register**, and **phase evidence**. The current records also define **Finding**, **limitation**, **disposition**, **controlled meaning**, and **acceptance**. A **sequence nonconformance** records an operation with authority when the project did not complete the necessary governance sequence before that operation. **Retrospective authority** is a record after an operation that claims that missing authority was available before the operation. It cannot give that authority. |
...
| Worktree retirement | **Worktree retirement** removes one worktree through the complete procedure in the recovery policy. **Accepted-history containment** shows that the accepted commit contains the specified commit. |
| Worktree retirement evidence | **Tracked cleanliness** is a Git state with no tracked index or worktree change. In this state, the Git index has no `assume-unchanged` or `skip-worktree` value. A **local-state inventory** identifies each local file that is not in the Git index. A **retirement plan** classifies that exact inventory. A **retirement audit** examines the plan without removal. |
| Types for worktree retirement | The 5 types are **authoritative local source**, **retained evidence**, **rebuildable cache/generated state**, **temporary disposable state**, and **ambiguous or uniquely owned state**. **Historical losslessness** is the result of a completed operation that kept all tracked work, authoritative local source, retained evidence, and items with no other copy. |
...
| **Classify** | Put a validation result with `FAIL` in one failure class that the testing policy gives. Put one local-state inventory item in one of the 5 types that the recovery policy gives. |
