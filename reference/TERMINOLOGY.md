...
| Documentation structure | **Technical Documentation Profile** is the title for `TT-DOC-001`. The **Technical Documentation Management Plan (TDMP)** is the complete lifecycle section in the Engineering Policy. A **technical document** supplies technical information for a stated purpose. A **controlled technical document** has an accepted **controlled baseline**. Its lifecycle state is identifiable from Git and applicable canonical records. A **technical-document lifecycle** controls a technical document from its **document need** through historical preservation. A **publication boundary** identifies the repository integration or other route that makes accepted information available. **Documentation profile**, **document class**, **canonical document**, **canonical prose**, **changed prose**, and **live document** are TrackTemplate document terms. **Frozen history**, **logical unit**, **complete logical unit**, **writing checklist**, **project plan**, and **technical provenance** are also document terms. |
| Documentation change | A **material change** changes technical meaning, authority, required use, ownership, scope, status, evidence, or a controlled instruction. A **non-material correction** changes none of those items. A **superseded document** has an accepted replacement that owns its current information. A **retired document** owns no required current information. A **failed experimental candidate** did not become a current controlled baseline. Preserve its exact receipt and proposal status. It has no current authority. **Migration**, **concision**, **wording**, **controlled writing**, **human interface**, **material edit**, **documentation simplification**, **readability**, **Learning from Experience (LFE)**, and **LFE record** are also TrackTemplate change terms. |
...
| Software operation | A **command invocation** contains an **executable**, **arguments**, and a **working directory**. A tool process uses a **command** to start a local operation. A **version** identifies a tool, schema, or source state. A **development tool** is not a **product runtime dependency**. A **development-toolchain preflight** checks declared prerequisites before a workflow stage starts. A **supported fallback** is a declared alternative that has authority and a prerequisite check. A **diagnostic** contains information about a tool result. **Application cancellation** stops one TrackTemplate application command at an explicit **cancellation point**. It does not stop the application process. A cancellation point is a caller-owned check at which the command can stop. |
