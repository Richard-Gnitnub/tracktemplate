...
| Worktree retirement | **Worktree retirement** removes one registered worktree after the recovery policy gives a complete procedure. **Accepted-history containment** proves that the accepted commit contains the target commit, and **tracked cleanliness** means that the target has no tracked index or worktree change. A **local-state inventory** contains each ignored and non-ignored local-only file and its identity, and a **retirement plan** classifies that exact inventory. A **retirement audit** checks the plan without removal. An **authoritative local source** and **retained evidence** need preservation. **Ambiguous or uniquely owned state** stops retirement, while **rebuildable cache/generated state** and **temporary disposable state** need a proved disposal basis. |
