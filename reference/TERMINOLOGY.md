...
| Project concepts | A **programme** is the current TrackTemplate Core programme or the later Layout Editor programme. **Architecture** is the accepted design and ownership structure in `reference/ARCHITECTURE.md`. **Persistence** is the FreeCAD data and document boundary that the architecture owns. This boundary stays through save and reopen. **Dependency direction** is the permitted direction for a dependency from one TrackTemplate source layer or component to another. |
...
| Governance state | **Status**, **phase**, **phase exit**, **risk**, **decision**, **decision register**, and **phase evidence** have the meanings in the current records. **Finding**, **limitation**, **disposition**, **controlled meaning**, and **acceptance** also use the meanings in those records. A **safety/risk panel** is the Level 3 review in the Engineering Policy. |
...
| Documentation structure | **Technical Documentation Profile** is the title for `TT-DOC-001`. The **Technical Documentation Management Plan (TDMP)** is the complete lifecycle section in the Engineering Policy. A **technical document** supplies technical information for a stated purpose. A **controlled technical document** has an accepted **controlled baseline**. Its lifecycle state is identifiable from Git and applicable canonical records. |
| Documentation lifecycle | A **technical-document lifecycle** controls a technical document from its **document need** through historical preservation. A **publication boundary** identifies the repository integration or other route that makes accepted information available. **Documentation profile**, **document class**, **canonical document**, **canonical prose**, **changed prose**, and **live document** are TrackTemplate document terms. **Frozen history**, **logical unit**, **complete logical unit**, **writing checklist**, **project plan**, and **technical provenance** are also document terms. |
| Documentation change | A **material change** changes technical meaning, authority, necessary use, ownership, scope, status, evidence, or a controlled instruction. A **non-material correction** changes none of those items. A **superseded document** has an accepted replacement that owns its current information. A **retired document** owns no necessary current information. |
| Failed candidate | A **failed experimental candidate** did not become a current controlled baseline. Preserve its exact receipt and proposal status. It has no current authority. |
| Documentation change terms | **Migration**, **concision**, **wording**, **controlled writing**, **human interface**, **material edit**, **documentation simplification**, **readability**, **Learning from Experience (LFE)**, and **LFE record** are also TrackTemplate change terms. |
...
| Assurance roles | **Evidence**, **validation**, **validator**, **reviewer**, **author**, **implementing agent**, and **agent** are TrackTemplate review terms. The **Technical Author Lead** manages technical-document authoring, delivery, and maintenance coordination. **Documentation reviewer**, **independent reviewer**, **independent quality reviewer**, **independent security/recovery reviewer**, and **independent documentation reviewer** are also review terms. |
| Assurance operations | **Independent review**, **quality review**, **security/recovery review**, **documentation review**, and **documentation alignment** are review terms. A **recovery validator** validates each recovery control. An **assurance** connects evidence and review to a specified claim. **Agent context** is text that an agent uses for a task. |
| Assurance workflow | **Workflow**, **Git workflow**, **context recovery**, **routing**, **handoff**, **skill**, **skill catalog**, and **documentation skill routing** are workflow terms. **Workflow responsibility**, **competing responsibility**, **authority boundary**, and **non-ownership boundary** are also workflow terms. The TDMP owns the complete technical-document lifecycle. The **Documentation Review lifecycle** is `author → freeze scope → one Documentation Review → optional exact reviewed correction once → one final deterministic validation → complete or owner stop`. It is the bounded authoring and review part of the complete lifecycle. The Documentation Review is the only linguistic conformance review. |
| Assurance result | **Conformance review**, **conformance scope**, **semantic control**, **preservation audit**, **complete result**, **review result**, **assessment**, and **machine-verifiable assurance** are assurance terms. A Documentation Review result has one **verdict**. It is `ACCEPT`, `APPROVED_WITH_EXACT_CORRECTIONS`, or `BLOCKED`. An **exact correction** supplies reviewed replacement wording for a verified **preimage**. An **accepted document identity** identifies the document content from the last accepted review. |
| Controlled baseline | An **accepted/controlled baseline** is the exact content of a technical document with acceptance from the applicable authority. That acceptance follows the one necessary Documentation Review, permitted adjustment, and final deterministic validation. |
...
| Software operation | A **command invocation** contains an **executable**, **arguments**, and a **working directory**. A tool process uses a **command** to start a local operation. A **version** identifies a tool, schema, or source state. A **diagnostic** contains information about a tool result. |
| Development tools | A **development tool** is not a **product runtime dependency**. A **development-toolchain preflight** does a check of the declared requirements for a workflow operation before that operation starts. A **supported fallback** is a declared alternative with project authority and a check of its requirements. |
| Application cancellation | **Application cancellation** stops one TrackTemplate application command at an explicit **cancellation point**. It does not stop the application process. A cancellation point is a check that the caller controls. At this check, the command can stop. |
...
| Performance result | A **journey remainder** is the full-journey CPU or wall time minus the measured operation times. **Cleanup** is product work that releases temporary resources and keeps the necessary state after an operation. A **cleanup result** is that state. Cleanup can occur in a measured journey or during Teardown. Cleanup and Teardown have different meanings. |
| Performance invariants | A **discrete invariant** is an exact object, recompute, cache, lifecycle, or cleanup result. **Measurement noise** is variation that the product change does not cause. |
...
| **Author** | Create or change technical-document content through the TDMP. |
...
| **Retire** | End a technical document's current authority after evidence shows that it owns no necessary current information. |
...
| **Supersede** | Make an accepted replacement the current authority for a technical document's necessary information. |
