...
| Governance state | **Status**, **phase**, **phase exit**, **risk**, **decision**, **decision register**, and **phase evidence** have the meanings in the current records. **Finding**, **limitation**, **disposition**, **controlled meaning**, and **acceptance** also use their current-record meanings. A **sequence nonconformance** records an authorised operation that did not complete its mandatory governance sequence. **Retrospective authority** is a record after an operation that claims prior authority. The required authority did not exist before the operation. |
...
| Worktree retirement | **Worktree retirement** removes one worktree that `git worktree list` reports after the recovery policy gives a complete procedure. **Accepted-history containment** shows that the accepted commit contains the target commit. **Tracked cleanliness** means that the target has no tracked index or worktree change and no `assume-unchanged` or `skip-worktree` index flag. A **local-state inventory** identifies each local-only file, a **retirement plan** classifies that exact inventory, and a **retirement audit** examines the plan without removal. The five types are **authoritative local source**, **retained evidence**, **rebuildable cache/generated state**, **temporary disposable state**, and **ambiguous or uniquely owned state**. **Historical losslessness** means that a completed operation kept all authoritative or unique local state. |
...
| **Classify** | Put a failed check in one failure class that the testing policy gives. Put one local-state inventory item in one of the five types that the recovery policy gives. |
