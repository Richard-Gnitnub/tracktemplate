...
| Documentation change | A **material change** changes technical meaning, authority, required use, ownership, scope, status, evidence, or a controlled instruction. A **non-material correction** changes none of those items. A **superseded document** has an accepted replacement that owns its current information. A **retired document** owns no required current information. |
...
| Controlled baseline | An **accepted/controlled baseline** is the exact content of a technical document with acceptance from the applicable authority. That acceptance follows the one required Documentation Review, permitted adjustment, and final deterministic validation. |
...
| Development tools | A **development tool** is not a **product runtime dependency**. A **development-toolchain preflight** checks the declared requirements for a workflow operation before that operation starts. A **supported fallback** is a declared alternative with project authority and a check of its requirements. |
...
| Performance result | A **journey remainder** is the full-journey CPU or wall time minus the measured operation times. **Cleanup** is product work that releases temporary resources and leaves the required retained state after an operation. A **cleanup result** is that retained state. Cleanup can occur in a measured journey or during Teardown. Cleanup and Teardown have different meanings. |
...
| **Retire** | End a technical document's current authority after evidence shows that it owns no required current information. |
...
| **Supersede** | Make an accepted replacement the current authority for a technical document's required information. |
