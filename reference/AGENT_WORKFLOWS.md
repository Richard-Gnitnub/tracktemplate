...
Use it to review a proposed or completed TrackTemplate change for:

- unnecessary complexity or speculative abstractions;
- duplicated logic, policy or documentation;
- broad rewrites unrelated to the request;
- misleading or repetitive comments;
- FreeCAD-specific defensive code that appears redundant but is not understood;
- weak validation, changed oracles without evidence, and unsupported completion
  claims;
- accidental changes to railway semantics, persistence, transactions,
  provenance or output status.

The skill is deliberately instruction-only. It does not run automatic cleanup,
assign an “authenticity score”, ban phrases or rewrite files in bulk. Those
mechanisms can create false positives and remove legitimate FreeCAD, railway or
licensing context.

## Invocation

In Codex CLI or the IDE extension, invoke the skill explicitly with:

```text
$tracktemplate-quality-review
```

Codex may also select it implicitly when a request clearly asks for review,
cleanup, simplification, documentation polish, maintainability assessment or a
pre-completion quality check.

Explicit invocation is preferable for release, phase-gate, large refactor,
licensing, persistence, export and performance work.
...
- Treat new skill behaviour as repository guidance: review it like code, keep the
  diff narrow and run the documentation/link controls.
- When a skill uncovers a durable project lesson, append it to
  `LEARNING_FROM_EXPERIENCE.md` only after the lesson has evidence and leads to
  an accepted reusable adaptation.

## Quality-review result

A skill review should report:

1. **Decision:** pass, pass with findings, or blocked.
2. **Findings:** ordered by impact, with exact paths or symbols.
3. **Validation:** checks actually run and their results.
4. **Residual uncertainty:** GUI, FreeCAD, performance, provenance or licensing
   evidence still required.
5. **Scope check:** confirmation that unrelated files and behaviour remained
   unchanged, or an explicit exception.

A clean prose style is not enough for a pass. Railway correctness,
recoverability, evidence quality and project authority remain controlling.